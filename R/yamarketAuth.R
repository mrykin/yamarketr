yamarketAuth <- function(Login = NULL, NewUser = FALSE, TokenPath = getwd()) {
  ...
  browseURL(paste0("https://oauth.yandex.ru/authorize?response_type=code&client_id=365a2d0a675c462d90ac145d4f5948cc&redirect_uri=https://mrykin.github.io/yamarketr/getToken/&force_confirm=", as.integer(NewUser), ifelse(is.null(Login), "", paste0("&login_hint=", Login))))
  # запрашиваем код
  ...
  token_raw <- httr::POST("https://oauth.yandex.ru/token", body = list(grant_type="authorization_code",
                                                                       code = temp_code,
                                                                       client_id = "edef5009ede4406daa4abb9d7b3b77eb",
                                                                       client_secret = "f2074f4c312449fab9681942edaa5360"), encode = "form")
  # парсим токен
